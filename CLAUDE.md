...
- **Beware per-tick rebuilds.** The countdown ticks every 100ms. Derived providers must return values that compare equal between ticks (`formattedTimeProvider` only changes once a second unless `showMilliseconds` is on); a provider that changes on every tick repaints the screen ten times a second *and* makes `pumpAndSettle` in the widget tests run until it times out.
